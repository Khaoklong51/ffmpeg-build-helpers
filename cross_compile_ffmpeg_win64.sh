...
check_missing_packages () {
  ...
  local check_packages=('ragel' 'curl' 'pkg-config' 'make' 'git' 'svn' 'gcc' 'autoconf' 'automake' 'nasm' 'cvs' 'flex' 'bison' 'makeinfo' 'g++' 'ed' 'pax' 'unzip' 'patch' 'wget' 'xz' 'nasm' 'gperf' 'autogen' 'bzip2' 'realpath' 'meson' 'clang' 'python')
  ...
  if [[ -n "${missing_packages[@]}" ]]; then
    ...
    apt_pkgs='subversion ragel curl texinfo g++ ed bison flex cvs nasm automake libtool autoconf gcc cmake git make pkg-config zlib1g-dev unzip pax nasm gperf autogen bzip2 autoconf-archive p7zip-full meson clang'
    ...
    case "$DISTRO" in
      ...
      debian)
        ...
        ;;
      *)
        echo "for OS X (homebrew): brew install ragel wget cvs nasm autogen automake autoconf cmake libtool xz pkg-config nasm bzip2 autoconf-archive p7zip coreutils meson llvm" # if edit this edit docker/Dockerfile also :|
        echo "   and set llvm to your PATH if on catalina"
        echo "for RHEL/CentOS: First ensure you have epel repo available, then run $ sudo yum install ragel subversion texinfo libtool autogen gperf nasm patch unzip pax ed gcc-c++ bison flex nasm automake autoconf gcc zlib-devel cvs bzip2 cmake3 -y"
  ...
  # doing the cut thing with an assigned variable dies on the version of nasm I have installed (which I'm pretty sure is the RHEL default)
...
EOF
  ...
  1 ) compiler_flavors=win64 ;;
  2 ) compiler_flavors=native ;;
...
install_cross_compiler() {
  ...
  cd cross_compilers
    ...
    echo "Starting to download and build cross compile version of clang [requires working internet access] with thread count $gcc_cpu_count..."
    echo ""

    # --disable-shared allows c++ to be distributed at all...which seemed necessary for some random dependency which happens to use/require c++...
    local zeranoe_script_name=mingw-w64-build-r22.local
    local zeranoe_script_options="--mingw-w64-ver=git --cpu-count=$gcc_cpu_count \
    --disable-shared --clean-build --verbose --allow-overwrite --threads=winpthreads \
    --gcc-ver=svn --enable-nls --enable-gendef --enable-widl --binutils-ver=git" 
...
do_cmake() {
  ...
  if [ ! -f $touch_name ]; then
    ...
    if [[ $compiler_flavors != "native" ]]; then
      local command="${build_from_dir} -DENABLE_STATIC_RUNTIME=1 -DBUILD_SHARED_LIBS=0 \
      -DCMAKE_SYSTEM_NAME=Windows -DCMAKE_FIND_ROOT_PATH=$mingw_w64_x86_64_prefix \
      -DCMAKE_FIND_ROOT_PATH_MODE_PROGRAM=NEVER -DCMAKE_FIND_ROOT_PATH_MODE_LIBRARY=ONLY \
      -DCMAKE_FIND_ROOT_PATH_MODE_INCLUDE=ONLY -DCMAKE_RANLIB=${cross_prefix}ranlib \
      -DCMAKE_C_COMPILER=${cross_prefix}gcc -DCMAKE_CXX_COMPILER=${cross_prefix}g++ \
      -DCMAKE_RC_COMPILER=${cross_prefix}windres -DCMAKE_INSTALL_PREFIX=$mingw_w64_x86_64_prefix $extra_args"
    else
    ...
    echo "doing ${cmake_command}  -G\"Ninja\" -B build -DCMAKE_BUILD_TYPE=Release $command"
    nice -n 5  ${cmake_command} -G"Ninja" -B build -DCMAKE_BUILD_TYPE=Release $command || exit 1
    touch $touch_name || exit 1
  fi
}
...
build_bzip2() {
  download_and_unpack_file $bzip2_tar
...
build_iconv() {
  download_and_unpack_file $iconv_git 
  cd libiconv-1.16
...
build_libtiff() {
  ...
  do_git_checkout_and_make_install $libtiff_git
  sed -i.bak 's/-ltiff.*$/-ltiff -llzma -ljpeg -lz/' $PKG_CONFIG_PATH/libtiff-4.pc # static deps
}

build_libtensorflow() {
  do_git_checkout_and_make_install $tensorflow_git
}

build_glib() {
  export CPPFLAGS="$CPPFLAGS -DLIBXML_STATIC -liconv" # gettext build...
  do_git_checkout_and_make_install  $gettext_tar
  reset_cppflags
  download_and_unpack_file $glib_git glib-2.64.3
  cd glib-2.64.3
...
build_libtesseract() {
  ...
  cd tesseract-git
    if [[ $compiler_flavors != "native"  ]]; then
      apply_patch file://$patch_dir/tesseract-mingw-std-threads.patch
      generic_configure "--disable-openmp --enable-shared=no --disable-doc --with-curl=no"
      do_make_and_make_install
      ...
      generic_configure_make_install "--enable-shared=no --disable-doc --with-curl=no"
...
build_glew() {
  download_and_unpack_file $glew_tgz glew-glew-2.2.0
  cd glew-glew-2.2.0/build
    local cmake_params=""
    if [[ $compiler_flavors != "native" ]]; then
...
build_libffi() {
  do_git_checkout_and_make_install $libffi_git
}
...
build_wayland() {
  ...
  -Ddocumentation=false -Dtests=false --buildtype=release --default-library=static . build"
  cd wayland-git
    do_meson "$meson_options"
    do_ninja_and_ninja_install
  cd ..
...
build_wayland_protocol() {
  ...
  local meson_options="--prefix=${mingw_w64_x86_64_prefix} --libdir=${mingw_w64_x86_64_prefix}/lib \
  -Dtests=false --buildtype=release --default-library=static . build"
...
build_harfbuzz() {
  ...
  build_freetype "--with-harfbuzz=no" $new_build # Check for initial or new freetype or force rebuild if needed
  local new_freetype=$?
  local meson_options="--prefix=${mingw_w64_x86_64_prefix} --libdir=${mingw_w64_x86_64_prefix}/lib \
  -Dfreetype=enabled -Ddocs=disabled -Dicu=disabled --buildtype=release --default-library=static . build"
  if $new_build || [ $new_freetype = 0 ]; then # 0 is true
    ...
    cd harfbuzz-git
    export LDFLAGS="-lbrotlidec -lbrotlienc -lbrotlicommon -lz -lpthread" # :|
    if [[ $compiler_flavors != "native" ]]; then
      get_local_meson_cross_with_propeties # Need to add flags to meson properties; otherwise ran into some issues
      meson_options+=" --cross-file=meson-cross.mingw.txt"
    fi
...
build_fontconfig() {
  do_git_checkout $fontconfig_tar fontconfig-git
  cd fontconfig-git
    export LDFLAGS="-lbrotlidec -lbrotlienc -lbrotlicommon -lz -lpthread"
...
build_libnettle() {
  download_and_unpack_file $libnettle_git nettle-git
  cd nettle-git
...
build_openssl-1.0.2() {
  download_and_unpack_file $openssl_1_0_2_tar
  cd openssl-1.0.2p
...
build_openssl-1.1.1() {
  download_and_unpack_file $openssl_1_1_1_tar
  cd openssl-1.1.1
    export CC="${cross_prefix}gcc"
    export AR="${cross_prefix}ar"
    export RANLIB="${cross_prefix}ranlib"
...
build_mpg123() {
  do_svn_checkout $mpg123_svn mpg123-svn r5008 # avoid Think again failure
  cd mpg123-svn
    generic_configure
    do_make_and_make_install
  cd ..
...
build_opencv() {
  ...
  download_and_unpack_file opencv-4.10.0
  mkdir -p opencv-4.10.0/build
  cd opencv-4.10.0
     apply_patch file://$patch_dir/opencv.detection_based.patch -p1
  cd ..
...
build_libflite() {
  ...
  download_and_unpack_file $libflite_git flite-2.1-release
...
build_librubberband() {
  do_git_checkout $rubberband_git rubberband-git 18c06ab8c431854056407c467f4755f761e36a8e
  cd rubberband-git
...
build_fribidi() {
  ...
  cd fribidi-git
  local meson_options="-Ddeprecated=false -Ddocs=false"
  mkdir build
    if [[ $compiler_flavors != "native" ]]; then
      get_local_meson_cross_with_propeties # Need to add flags to meson properties; otherwise ran into some issues
      meson_options+=" --cross-file=meson-cross.mingw.txt"
...
build_libaom() {
  ...
  mkdir -p aom-git/aom_build
  cd aom-git/aom_build
...
build_avisynth() {
  ...
  mkdir -p avisynth-git/avisynth-build
  cd avisynth-git/avisynth-build
...
build_libopenh264() {
  ...
  cd openh264_git
    ...
    if [[ $compiler_flavors == "native" ]]; then
      ...
      do_make "$make_prefix_options OS=mingw_nt ARCH=$arch ASM=nasm install-static"
    fi
  cd ..
}
...
build_libx264() {
  ...
  if [[ $prefer_stable = "n" ]]; then
    ...
    do_git_checkout "$x264_git" $checkout_dir "origin/master" 
  else
    do_git_checkout "$x264_git" $checkout_dir  "origin/stable" 
  fi
  cd $checkout_dir
...
build_libjpeg_turbo() {
  ...
  cd libjpeg-turbo-git
    local cmake_params="-DENABLE_SHARED=0 -DCMAKE_ASM_NASM_COMPILER=nasm"
    if [[ $compiler_flavors != "native" ]]; then
...
while true; do
  case $1 in
    -h | --help ) echo "available option=default_value:
      ...
      --compiler-flavors=[win64,native] [default prompt, or skip if you already have one built]
...
if [[ $compiler_flavors == "win64" ]]; then
  echo
